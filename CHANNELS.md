...
| `404 Not Found` | A `cnew` request names a target that is missing, has an invalid client-supplied path, or uses an unsupported file type. | No usable channel is created; the connection is closed. Correct the requested target. |
...
| `500 Internal Server Error` | The selected source is unreadable or fails JPEG 2000 validation, or the server encounters another internal failure such as being unable to generate a channel ID. The response body identifies the failure category. | The connection and channel are closed. An unreadable or invalid source normally requires correcting the repository file. |
