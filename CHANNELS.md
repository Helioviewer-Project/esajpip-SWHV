...
This document describes esajpip's HTTP connection and JPIP channel lifecycle.
It is an implementation guide, not a description of every transport and
session form allowed by JPEG 2000 Part 9.

## Terms and source layout

A **connection** is one accepted TCP socket. A **channel** is the longer-lived
JPIP session identified by the `cid` returned in `JPIP-cnew`. A channel may use
more than one connection during its lifetime.
...
descriptor is then owned either by the dispatcher, by a `ChannelInbox`, or by
the channel thread. Each transfer has one owner.

One channel thread exclusively owns its `FileManager`, `ImageIndex` and linked
JPX graph, `DataBinServer`, cache model, traversal state and response buffer.
...
parent: pending Connection + absolute identification deadline
    ...
    +-- clearly not supported JPIP GET traffic --------> close
    |
    v
...
The child repeats the bounded recognition after receiving the descriptor. This
derives the routing fields from the child-owned socket instead of passing
parsed request state between processes.
...
closed. This keeps the supported profile to one active response and at most one
waiting connection per channel.
...
connection wakes the thread; the current child-side descriptor is closed and
the replacement becomes current. This supports both JHelioviewer's persistent
socket and clients whose HTTP implementation opens a new socket for a later
request.
...
different HTTP connection. Keeping state with the channel therefore matches
the protocol model. esajpip deliberately retains a smaller profile than the
full standard: HTTP `GET`, one target, one channel thread, and serialized
...
- `0` and `-1` retain the existing behavior of disabling the communication
  timeout.
...
safely preserve the channel because its cache model may already describe bytes
which the client did not receive completely.
...
If the serving child is restarted, its per-channel state cannot be recovered.
The new child closes inherited descriptor copies and shuts down already admitted
connections so clients can establish new channels. Pending, unidentified
connections remain governed by the parent admission deadline.
