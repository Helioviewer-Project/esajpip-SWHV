...
InitialRequest InspectInitialRequest(int fd) {
    ...
    if (length < 0 && (errno == EAGAIN || errno == EWOULDBLOCK))
        return {REQUEST_PENDING, false, false, 0};
    if (length <= 0)
        return {REQUEST_REJECTED, false, false, 0};
...
InitialRequest ClassifyInitialRequest(const char *line, size_t length) {
    ...
    if (cclose) {
        ...
        return ParseChannel(*route, &channel)
                   ? InitialRequest{REQUEST_ACCEPTED, false, handled, channel}
                   : InitialRequest{REQUEST_REJECTED, false, handled, 0};
    }
    if (cnew)
        return {REQUEST_ACCEPTED, true, handled, 0};
    uint64_t channel;
    if (cid && ParseChannel(*cid, &channel))
        return {REQUEST_ACCEPTED, false, handled, channel};
    return {REQUEST_REJECTED, false, handled, 0};
}
