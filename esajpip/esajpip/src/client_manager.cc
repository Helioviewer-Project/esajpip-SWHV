    ...
    while (!pclose) {
        ...
        if (req.mask.items.cclose) {
            if (!is_opened) {
                ...
                index_manager.CloseImage(image_index);
            ...
            if (is_opened) {
                ...
                if (!index_manager.OpenImage(file_name, &image_index))
                    ERROR("The image file '" << file_name << "' can not be read");
                else {
                    is_opened = true;
                    data_server.Reset();
                    if (!data_server.SetRequest(index_manager, image_index, req))
            ...
            if (!is_opened) {
                ...
                if (req.parameters["cid"] != channel) {
                    ...
                    LOG(err_msg);
                } else if (!data_server.SetRequest(index_manager, image_index, req))
                    ERROR("The server can not process the request");
                else {
                    sock_stream << http::Response(200)
                                << (send_gzip ? head_data_gzip.str() : head_data.str())
        ...
        if (pclose) {
            ...
            if (!send_gzip)
                for (bool last = false; !last;) {
                    ...
                    if (!data_server.GenerateChunk(index_manager, image_index, buf, &chunk_len, &last)) {
