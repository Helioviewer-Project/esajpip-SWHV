...
| `tid` | Reduced | Does not affect target selection. A successful request carrying this field receives `JPIP-tid: 0`, indicating that the server does not assign a stable target identifier. If the supplied value is not `0`, the server disregards `model` because it belongs to a different target identity. |
...
| Other standard fields | Not supported | Fields including `subtarget`, `qid`, `comps`, `srate`, `roi`, `layers`, `quality`, `align`, `wait`, `drate`, `tpmodel`, `need`, `tpneed`, `mset`, `csf`, upload, capability, timed-request, and delivery-control fields have no implemented semantics. |
