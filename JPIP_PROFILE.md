...
| `fsiz`, `roff`, `rsiz` | Supported profile | Select resolution and a rectangular window of interest. JHelioviewer supplies these fields together. When omitted, `roff` defaults to `(0,0)` and `rsiz` extends to the lower-right corner. `round-up`, `round-down`, and `closest` are recognized on `fsiz`. The region is cropped to the selected image resolution; empty and non-overlapping regions are rejected. Server-adjusted window response headers are not generated. |
