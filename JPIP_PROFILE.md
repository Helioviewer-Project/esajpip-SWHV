...
| `len` | Supported | Limits the number of JPP data-bin bytes generated for the response; the three-byte EOR is excluded from this limit. When omitted, the server sends all data relevant to the request. A client can issue further requests using the same channel and cache model. Negative values are rejected. |
