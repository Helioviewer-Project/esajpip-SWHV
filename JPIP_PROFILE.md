...
| Errors | Reduced | Requests that reach a channel receive `400` for malformed supported fields, `404` for a missing target, `431` for a request head over 4 KiB, or `500` for other failures. The dispatcher returns `503 JPIP channel unavailable` when the channel is unknown, has ended, or already has a replacement connection waiting. Errors terminate the connection and, after channel creation, the channel. Traffic rejected during initial inspection is closed without an HTTP response. The complete JPIP correction-header model is not implemented. |
