...
| `fsiz`, `roff`, `rsiz` | Supported profile | Select resolution and a rectangular window of interest. JHelioviewer supplies these fields together. `round-up`, `round-down`, and `closest` are recognized on `fsiz`. The region is cropped to the selected image resolution; empty and non-overlapping regions are rejected. Server-adjusted window response headers are not generated. |
| `stream` | Reduced | Selects one codestream or one inclusive numeric range, with selector values capped at 100,000. At most 100,001 codestreams may be selected across all `stream` and `context` fields in one request. The implementation also retains the historical descending-range extension used by existing clients. Lists and the complete standard sampled-range syntax are not supported. |
